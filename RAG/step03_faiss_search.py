"""
step03_faiss_search.py — build a local FAISS index from embedded chunks and
...
once step04_hybrid_search.py promotes a matched child back to its parent. The
...
Mechanism:
1. Load embedded_child_chunks.jsonl (chunk text + metadata + embedding, from step02_embed.py).
2. Build a FAISS IndexFlatIP — brute-force exact inner-product search.
   ...
   in step02_embed.py), so inner product == cosine similarity here; no extra
...
4. Embed the query with the SAME model/settings as the chunks — imported
   directly from step02_embed.py, so there's one source of truth for the embedding
   config rather than two copies that could drift apart.

Requires: pip install faiss-cpu

Usage:
    python step03_faiss_search.py --build              # build the index (run after each step02_embed.py run)
    python step03_faiss_search.py <query text>          # search it — note: this standalone search
                                                  # returns raw CHILD text, no parent promotion.
                                                  # Promotion only happens in step04_hybrid_search.py.
"""
...
from step02_embed import embed_text  # reuse the exact embedding call/config used during ingestion
...
def build_index():
    ...
    with METADATA_PATH.open("w", encoding="utf-8") as f:
        for c in chunks:
            ...
            # write whichever this record actually has, so step04_hybrid_search.py's
...
def load_index():
    if not INDEX_PATH.exists() or not METADATA_PATH.exists():
        raise RuntimeError("Index not built yet — run `python step03_faiss_search.py --build` first.")
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--build":
        ...
        print("Usage:")
        print("  python step03_faiss_search.py --build")
        print("  python step03_faiss_search.py <query text>")
