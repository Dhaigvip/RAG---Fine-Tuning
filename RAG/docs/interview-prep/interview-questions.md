...
- **Reranking**: retrieve wide, rerank down with a cross-encoder — bi-encoder similarity alone isn't precise enough for what actually goes to the LLM. Built with two interchangeable backends behind one interface (a local sentence-transformers cross-encoder, and Amazon Bedrock's Rerank API kept dormant but intact) after an AWS Organizations Service Control Policy blocked Bedrock's rerank models outright — a real example of designing around an infrastructure/governance constraint instead of hard-coding to one vendor. Also added a per-backend relevance-score threshold after live testing showed the reranker correctly scoring irrelevant candidates deep negative, while a naive fixed-top-k cutoff still handed them to the LLM anyway.
...
Concrete example from this build: after reranking, a fixed top-k cutoff was still handing downstream generation chunks the reranker itself had scored as confidently irrelevant (deep negative cross-encoder scores), whenever the corpus simply didn't contain k relevant chunks for a query. Fixed with an empirically-calibrated relevance threshold instead of a fixed count, so a query with no good match now legitimately returns fewer — even zero — chunks rather than forcing k anyway. It's the same principle as the retrieval-vs-generation-failure point above, caught one stage earlier: garbage retrieved is garbage in context no matter how good the generation prompt is.
