...
**Decision: implemented the relevance-threshold strategy**, per-backend since the two backends' scores aren't on the same scale (see `hybrid_search.py`'s `apply_min_score()`):
