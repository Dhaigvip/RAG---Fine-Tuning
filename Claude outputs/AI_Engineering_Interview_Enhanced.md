...
Self-attention lets every token look at every other token and decide how much to "attend to" each one, producing a weighted sum of values:

```
Attention(Q, K, V) = softmax(QK^T / √d_k) · V
```

where Q (query), K (key), and V (value) are all learned linear projections of the input, and dividing by `√d_k` keeps the dot products from growing too large before the softmax. This runs in O(n²) time in sequence length — every token attends to every other token — which is the direct reason long context windows get expensive fast (Q7), and the reason optimizations like PagedAttention (Q102) and Flash Attention exist.

**Multi-head attention** runs h parallel attention "heads," each with its own learned Q/K/V projections, so different heads can specialize — one might track syntax, another long-range topical relationships — before their outputs are concatenated back together. See Q175 for how this fits into the full Transformer block.
...
**Top-k sampling** restricts the candidate pool to only the k highest-probability tokens at each step, then samples from that fixed-size set. **Top-p (nucleus) sampling** instead keeps the smallest set of tokens whose cumulative probability reaches at least p, so the candidate pool size adapts automatically — narrower when the model is confident, wider when it's uncertain — which tends to produce more coherent output than a fixed k.

A common production default is top_k=40, top_p=0.95. The order of operations matters and is easy to get wrong in an implementation: temperature (Q3) scales the logits first, *then* top-k/top-p filters the candidate pool, *then* the actual sampling happens from what remains.
...
The complete building block, stacked N times to form the full model:
...
[Token Embedding] + [Positional Encoding]
     ...
     ▼
┌─────────────────────────────┐
│  Multi-Head Self-Attention   │  (Q13)
│           +                  │
│    Add & LayerNorm           │  ← residual (Q162) + LayerNorm (Q163)
├─────────────────────────────┤
│  Feed-Forward Network (MLP)  │  ← GELU activation (Q167), ~4x hidden expand
│           +                  │
│    Add & LayerNorm           │
└─────────────────────────────┘
     │
     ▼  (repeat N times — 32 blocks for a 7B model)
     ▼
[Output projection → vocabulary logits]
```

For a decoder (GPT/Claude-style), self-attention is causal/masked (Q16). For an encoder-decoder (T5-style), the decoder adds a cross-attention sub-layer between self-attention and the feed-forward network, attending to the encoder's output (Q172). This architecture scales remarkably well: more data plus more compute reliably yields better performance, which is the empirical basis for the scaling laws in Q18 and Q87.

Three variants matter most for interviews: **encoder-only** (bidirectional, e.g. BERT, your reranker's CrossEncoder), **decoder-only** (causal, e.g. GPT/Claude — the architecture that makes autoregressive generation from Q16 possible), and **encoder-decoder** (both, e.g. T5). Self-attention's O(n²) complexity (Q13) is why long context is expensive (Q7) and why PagedAttention (Q102) exists.
