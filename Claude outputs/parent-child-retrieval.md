...
**Testing**: `promote_to_parents()` is a pure function (plain data in/out, no AWS/model/FAISS dependency), unit-tested in `tests/test_hybrid_search.py` — basic mapping, dedup-keeps-highest-scoring-child, top_k-applied-after-dedup (the bug case above), and empty input. Verified locally against stubbed AWS/ML dependencies (no network access in my sandbox to install the real packages) — all 11 tests (7 existing + 4 new) pass, and a full `search()` dry run with fake candidates confirms the display/promotion path doesn't crash. Real `pytest` run on your machine against the actual packages is still the authoritative check.

**Not yet done**: real end-to-end run against your actual `child_chunks.jsonl` (embed → build index → query) hasn't been executed or verified against real data yet — that's the next step.
